---
name: kubectl
description: 使用 kubectl 管理 Kubernetes 集群资源。由于环境限制，必须通过指定 kubeconfig 文件路径来操作特定集群。
required_skills:
...
4.  **常用场景 SOP**：
    ...
    *   **资源操作 (需谨慎)**：
        *   在执行 `delete`, `scale`, `apply`, `edit` (不推荐edit) 等修改类操作前，**务必**向用户确认目标资源（Namespace 和 Name）。
