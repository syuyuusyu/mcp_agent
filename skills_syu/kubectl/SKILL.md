...
1.  **环境配置**：
    直接使用系统默认的 `kubectl` 配置与当前 context，无需显式指定 kubeconfig 路径。
...
3.  **非交互式限制**：
    ...
    *   若需在 Pod 内执行命令，去掉 `-it`，例如：`kubectl exec <pod_name> -- ls -la /app`。
    *   若需查看日志，使用 `--tail` 限制行数，例如：`kubectl logs <pod_name> --tail=200`。
...
4.  **常用场景 SOP**：
    ...
    *   **排查 Pod 问题**：
        1.  查看 Pod 状态: `kubectl get pod <pod_name> -n <ns>`
        2.  查看详细事件: `kubectl describe pod <pod_name> -n <ns>`
        3.  查看应用日志: `kubectl logs <pod_name> -n <ns> --tail=100`
        4.  查看上一容器日志(如果崩溃重启过): `kubectl logs <pod_name> -n <ns> --previous`
